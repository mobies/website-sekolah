...
const DataMapelForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const snapshot = await get(dbRef(database, `tenants/${tenantId}/references/mapel/${id}`));
        if (snapshot.exists()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.kode.trim()) {
      setError('Kode mapel harus diisi');
      return;
    }
    ...
    if (!formData.nama.trim()) {
      setError('Nama mapel harus diisi');
      return;
    }
    ...
    try {
      ...
      if (isEdit) {
        ...
        await update(dbRef(database, `tenants/${tenantId}/references/mapel/${id}`), dataToSave);
        ...
        await update(dbRef(database, `tenants/${tenantId}/references/mapel/${formData.kode}`), dataToSave);
        toast.fire({ icon: 'success', title: 'Data berhasil ditambah' });
      }

      navigate('/dashboard/referensi/mapel');
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <Container className="mt-4 mb-4" style={{ maxWidth: '600px' }}>
        <Row className="mb-4">
          <Col>
            <h2 className="fw-bold">{isEdit ? 'Edit' : 'Tambah'} Data Mapel</h2>
          </Col>
        </Row>
        ...
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3">
            <Form.Label className="fw-bold">Kode Mapel</Form.Label>
          ...
          <Form.Group className="mb-3">
            <Form.Label className="fw-bold">Nama Mapel</Form.Label>
            <Form.Control
              type="text"
          ...
          <Row className="gap-2">
            ...
            <Col xs="auto">
              <Button variant="secondary" onClick={() => navigate('/dashboard/referensi/mapel')} disabled={loading}>
                Batal
              </Button>
            </Col>
          </Row>
