...
interface ItemPembayaran {
  id: string;
  nama: string;
  createdAt?: number;
}

const ItemPembayaranList: React.FC = () => {
  const navigate = useNavigate();
  const { tenantId } = useTenant();
  const [data, setData] = useState<ItemPembayaran[]>([]);
  ...
  useEffect(() => {
    ...
    const dataRef = getDBRef(tenantId, 'references/pembayaran');
    const unsubscribe = onValue(
      dataRef,
      (snapshot) => {
        if (snapshot.exists()) {
          const items: ItemPembayaran[] = [];
  ...
  const handleDelete = async (id: string, nama: string) => {
    ...
    try {
      await remove(dbRef(database, `tenants/${tenantId}/references/pembayaran/${id}`));
    ...
    <DashboardLayout>
      <Container fluid className="mt-4 mb-4">
        <Row className="mb-4 align-items-center">
          <Col>
            <h2 className="fw-bold">Item Pembayaran</h2>
          </Col>
          <Col className="text-end">
            <Button variant="success" onClick={() => navigate('/dashboard/referensi/pembayaran/tambah')}>
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td>{item.nama}</td>
                  <td>
                    <Button size="sm" variant="primary" className="me-2" onClick={() => navigate(`/dashboard/referensi/pembayaran/edit/${item.id}`)}>
                      <FaEdit /> Edit
                    </Button>
...
export default ItemPembayaranList;
