...
const ItemSimpananForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const snapshot = await get(dbRef(database, `tenants/${tenantId}/references/simpanan/${id}`));
        if (snapshot.exists()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await update(dbRef(database, `tenants/${tenantId}/references/simpanan/${id}`), dataToSave);
        toast.fire({ icon: 'success', title: 'Data berhasil diupdate' });
      } else {
        await push(dbRef(database, `tenants/${tenantId}/references/simpanan`), dataToSave);
        toast.fire({ icon: 'success', title: 'Data berhasil ditambah' });
      }

      navigate('/dashboard/referensi/simpanan');
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <Container className="mt-4 mb-4" style={{ maxWidth: '600px' }}>
        <Row className="mb-4">
          <Col>
            <h2 className="fw-bold">{isEdit ? 'Edit' : 'Tambah'} Item Simpanan</h2>
          </Col>
        </Row>
        ...
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3">
            ...
            <Form.Control
              type="text"
              placeholder="cth: Tabungan Wisata"
              value={formData.nama}
          ...
          <Row className="gap-2">
            ...
            <Col xs="auto">
              <Button variant="secondary" onClick={() => navigate('/dashboard/referensi/simpanan')} disabled={loading}>
                Batal
              </Button>
            </Col>
          </Row>
...
export default ItemSimpananForm;
