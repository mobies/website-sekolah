...
interface ItemSimpanan {
  id: string;
  nama: string;
  createdAt?: number;
}

const ItemSimpananList: React.FC = () => {
  const navigate = useNavigate();
  const { tenantId } = useTenant();
  const [data, setData] = useState<ItemSimpanan[]>([]);
  ...
  useEffect(() => {
    ...
    const dataRef = getDBRef(tenantId, 'references/simpanan');
    const unsubscribe = onValue(
      dataRef,
      (snapshot) => {
        if (snapshot.exists()) {
          const items: ItemSimpanan[] = [];
  ...
  const handleDelete = async (id: string, nama: string) => {
    ...
    try {
      await remove(dbRef(database, `tenants/${tenantId}/references/simpanan/${id}`));
    ...
    <DashboardLayout>
      <Container fluid className="mt-4 mb-4">
        <Row className="mb-4 align-items-center">
          <Col>
            <h2 className="fw-bold">Item Simpanan</h2>
          </Col>
          <Col className="text-end">
            <Button variant="success" onClick={() => navigate('/dashboard/referensi/simpanan/tambah')}>
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td>{item.nama}</td>
                  <td>
                    <Button size="sm" variant="primary" className="me-2" onClick={() => navigate(`/dashboard/referensi/simpanan/edit/${item.id}`)}>
                      <FaEdit /> Edit
                    </Button>
...
export default ItemSimpananList;
