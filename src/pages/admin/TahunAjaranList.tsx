...
const TahunAjaranList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const dataRef = getDBRef(tenantId, 'references/tahun-ajaran');
  ...
  const handleDelete = async (id: string, nama: string) => {
    ...
    try {
      await remove(dbRef(database, `tenants/${tenantId}/references/tahun-ajaran/${id}`));
    ...
    <DashboardLayout>
      <Container fluid className="mt-4 mb-4">
        <Row className="mb-4 align-items-center">
          <Col>
            <h2 className="fw-bold">Tahun Ajaran</h2>
          </Col>
          <Col className="text-end">
            <Button variant="success" onClick={() => navigate('/dashboard/referensi/tahun-ajaran/tambah')}>
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <Button size="sm" variant="primary" className="me-2" onClick={() => navigate(`/dashboard/referensi/tahun-ajaran/edit/${item.id}`)}>
                      <FaEdit /> Edit
                    </Button>
